...
const BlogFormUpdate = (props) => {
    ...
    const updatePost = (e) => {
        ...
        try {
            ...
            let url = `https://api-hiddevblog.vercel.app/api/posts/${props.slug}`
            axios.patch(url, {
                title,
                body: finalText,
                category,
                    ...
                    <div className="w-full gap-2 justify-between flex flex-col">
                        <div className="w-full flex flex-col justify-between">
                            ...
                            <div className="px-2 flex flex-col w-max gap-2">
                                <div className="p-1 rounded-md text-xs border">
                                    <select id="countries" name="category" className="w-full" onChange={(e) => setCategory(e.target.value)}>
                                        <option key={props.categoryId} value={props.categoryId} selected><p>&#128193;</p> {props.category}</option>
                                        {categories.map((cat) => (
                                            <option key={cat._id} value={cat._id}><p>&#128193;</p> {cat.name}</option>
                                        ))}
                                    </select>
                                </div>
                            </div>
